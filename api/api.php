                ...
                if($cha_1['zt'] == 1) {
                    if ($cha_2['rwzl_zt'] == 0) {//入网资料
                        if ($jzxm == "" || $sfzh == "" || $mgz == "" || $sfz1 == "" || $sfz2 == "") {
                            ...
                            $DB->query("update mao_dindan set sjh='{$sjh}',xm='{$xm}',dz='{$dz}',xxdz='{$xxdz}',ly='{$ly}',jzxm='{$jzxm}',sfzh='{$sfzh}',mgz='{$mgz}',sfz1='{$sfz1}',sfz2='{$sfz2}' where M_id='{$mao['id']}' and id='{$cha_1['id']}'");
                            $result = array("code" => 0);
                            $result=array("code"=>0,"ddh"=>$cha_1['ddh']);
                            $_SESSION['shouji'] = $cha_1['sjh'];
                        }
                    } else {
                        $DB->query("update mao_dindan set sjh='{$sjh}',xm='{$xm}',dz='{$dz}',xxdz='{$xxdz}',ly='{$ly}' where M_id='{$mao['id']}' and id='{$cha_1['id']}'");
                        $result = array("code" => 0);
                        $result=array("code"=>0,"ddh"=>$cha_1['ddh']);
                        $_SESSION['shouji'] = $cha_1['sjh'];
                    }
